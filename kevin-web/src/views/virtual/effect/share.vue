<template>
  <div class="analysis-page">
    <el-card shadow="never" class="header-card"><h3>开放共享成效</h3><p>展示共享申请状态分布和开放共享处理情况。</p></el-card>
    <el-row :gutter="16">
...
export default {
  ...
  computed: { rows() { return [{ name: '申请状态数', value: (this.summary.applyStatusStats || []).length + ' 类' }, { name: '资源总数', value: this.summary.resourceCount || 0 }, { name: '实验室总数', value: this.summary.labCount || 0 }] } },
  ...
  methods: { renderChart() { this.chart = echarts.init(this.$refs.chart); this.chart.setOption({ title: { text: '申请状态分布' }, tooltip: {}, series: [{ type: 'pie', radius: '62%', data: this.summary.applyStatusStats || [] }] }) } }
}
</script>
<style scoped>
