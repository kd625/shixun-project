<template>
  <div class="analysis-page">
    <el-card shadow="never" class="header-card"><h3>资源利用率</h3><p>通过资源数量和类型结构展示资源利用基础指标。</p></el-card>
    <el-row :gutter="16">
...
export default {
  ...
  computed: { rows() { return [{ name: '资源总数', value: this.summary.resourceCount || 0 }, { name: '设备支撑数', value: this.summary.deviceCount || 0 }] } },
  ...
  methods: { renderChart() { this.chart = echarts.init(this.$refs.chart); this.chart.setOption({ title: { text: '资源结构' }, tooltip: {}, series: [{ type: 'pie', radius: ['42%', '68%'], data: this.summary.resourceTypeStats || [] }] }) } }
}
</script>
<style scoped>
